...
export async function getCreators() {
  if (isSupabaseConfigured) {
    ...
    if ((data ?? []).length === 0) {
      const { data: seeded, error: seedError } = await supabase.from('creators').insert(seedCreators).select('*').order('id')
...
export async function addCreator(creator) {
  ...
  const newCreator = { id: Date.now(), created_at: new Date().toISOString(), ...creator }
...
export async function updateCreator(id, creator) {
  if (isSupabaseConfigured) {
    const { data, error } = await supabase.from('creators').update(creator).eq('id', id).select().single()
    if (error) throw error
    return data
  }

  const creators = readLocalCreators()
  const updatedCreators = creators.map((c) => (String(c.id) === String(id) ? { ...c, ...creator } : c))
  writeLocalCreators(updatedCreators)
  return updatedCreators.find((c) => String(c.id) === String(id))
}
